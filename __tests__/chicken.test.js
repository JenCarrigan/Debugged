...
describe('Chicken', () => {
  it('should create and populate', (done) => {
    ...
    Coop.create({location:'Red Barn'}).then(coop => {
      ...
      Chicken.create({name:'Joey', coop}).then((chicken) => {
        ...
        Chicken.findById(chicken._id).then(pollo => {

          expect(chicken.coop.location).toBe('Red Barn');
  ...
  it('should create and populate - async/await', async () => {
    ...
    const pollo = await Chicken.findById(chicken._id);

    expect(pollo.coop).toEqual(coop._id);  
  });
});